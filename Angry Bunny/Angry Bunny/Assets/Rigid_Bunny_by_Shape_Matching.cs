...
public class Rigid_Bunny_by_Shape_Matching : MonoBehaviour
{
	...
	void Start()
	{
		...
		X = mesh.vertices;
		// Reference position of each vertex.
		Q = mesh.vertices;
	...
	void Update()
	{
		...
		// Run a simple particle system.
		for (int i = 0; i < V.Length; i++)
		{
			V[i] = linear_decay * V[i];
			V[i] = V[i] + g * dt;
			X[i] = X[i] + V[i] * dt;
		}

		// Perform simple particle collision.
		Collision(1 / dt);

		// Get new translation c.
